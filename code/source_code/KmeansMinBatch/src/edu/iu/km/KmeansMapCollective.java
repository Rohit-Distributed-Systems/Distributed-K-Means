...
public class KmeansMapCollective extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length < 6) {
			System.err.println(
					"KmeansMapCollective <batchSizeInPercent> <num of Centroids> <number of iteration> <workDir> <localDir for data> <localDir for application output>");
		...
		int numMapTasks = KMeansConstants.NUMBER_OF_MAPPERS;

		int iterations = Integer.parseInt(args[2]);
		Path workingDir = new Path(args[3]);
		String localDirData = args[4];
		String localDirOutput = args[5];
		String docFilesDir = localDirData + File.separator + KMeansConstants.DOCUMENT_DIR;
		String categoriesDir = localDirData + File.separator + KMeansConstants.CATEGORY_DIR;
		Configuration conf = getConf();
		FileSystem fs = FileSystem.get(conf);
		Path data = new Path(workingDir, KMeansConstants.DATA_DIR);
		Path documents = new Path(data, KMeansConstants.DOCUMENT_DIR);
		Path categories = new Path(data, KMeansConstants.CATEGORY_DIR);

		Utils.setupData(fs, docFilesDir, categoriesDir, documents, categories);

		Job job = Job.getInstance(conf, "mini-batch-kmeans");

		FileInputFormat.setInputPaths(job, new Path[] { documents });
		Path hadoopOutput = new Path(workingDir, "hadoopOutput");
		Path appOutput = new Path(workingDir, "appOutput");
